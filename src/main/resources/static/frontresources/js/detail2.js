$(document).ready(function() {
	...
	$("#loanAmount").blur(function() {
		...
		if (validateFunction.isEmpty(amount)) {
			...
			//alert(validateFunction.isCurrency(amount));
	...
	$("#save").click(function() {
        ...
        var js_dueId = $("#js_dueId").val();
        var income = $("#income").val();
        
        var options = {
            type: 'post',                     //post提交
            url: dynamicURL+'/front/borrow/apply',     //url
            ...
            success: function (data) {
                //服务器端返回处理逻辑
                if (data.success) {
                    window.location.href = "${dynamicURL }/front/borrow/check";
               } else {
                   layer.confirm("提交信息失败！", {
                        btn: ['退出当前页面', '重新填写'],
                      title: ""
                    }, function (index, layero) {
                        window.location.href = "${dynamicURL }/front/main";
                    }, function (index) {
                        window.location.href = "${dynamicURL }/front/borrow";
                    });
                }
            }
        }
        $("#applyid").ajaxSubmit(options);
	});
});
	/*$("#save").click(function() {
		errorCount = 0;
